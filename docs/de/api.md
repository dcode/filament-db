...
Bulk-Import von OrcaSlicer-Filamentbibliotheks-Presets (die Dateien unter `…/OrcaSlicer/system/OrcaFilamentLibrary/filament`), deren `inherits`-Ketten serverseitig aufgelöst werden. Die Kachel „OrcaSlicer-Bibliothek (Ordner)" auf der Import/Export-Seite bedient diesen Endpunkt per Ordner-Upload; die Auswahl durchsucht Unterverzeichnisse rekursiv und filtert Nicht-Filament-Presets heraus, sodass auch ein übergeordnetes Verzeichnis gewählt werden kann, das SOWOHL die Systembibliothek als auch den eigenen Benutzerprofil-Ordner enthält (z. B. der OrcaSlicer-Konfigurationsordner selbst) — nötig, wenn die `inherits`-Kette eines angepassten Benutzerprofils bis in den Systembaum zurückreicht.
...
- Namenskollisionen — für aktive wie gelöschte (Papierkorb-)Zeilen gleichermaßen (eine Papierkorb-Zeile gleichen Namens wird auf demselben Weg wiederhergestellt, auf dem eine aktive aktualisiert würde): Eine bestehende Variante desselben Eltern-Datensatzes wird per Diff aktualisiert bzw. wiederhergestellt und behält ihre Eltern-Verknüpfung (idempotenter Re-Import — eltern-gleiche vererbbare Skalare und Düsentemperaturen reisen zusätzlich im Payload mit, sodass eine veraltete lokale Überschreibung, die das Profil nicht mehr enthält, wieder auf Vererbung zurückgesetzt wird); ein bestehendes Wurzel-Filament wird an Ort und Stelle mit vollen aufgelösten Werten aktualisiert bzw. wiederhergestellt und nie umgehängt; eine bestehende Variante eines *anderen* Eltern-Datensatzes wird mit einem Fehler pro Profil übersprungen. Ein importiertes *Wurzel*-Profil, dessen Name zu einer bestehenden Variante gehört, wird ebenfalls mit einem Fehler pro Profil übersprungen (ein Update würde seine geplanten Kinder zu Varianten-von-Varianten machen).
